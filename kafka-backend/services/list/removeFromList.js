...
async function removeFromList(msg, callback) {
    let response = {};
    let err = {};
        ...
        if (!user || !list) {
            ...
            switch (msg.type) {
                ...
                case "subscriber":
                    ...
                    addCondition = user.subscribed_lists.includes(msg.list_id) && list.subscribers.includes(msg.user_id) ;
                    break;
            }
            console.log(addCondition);
            if (addCondition) {
                userUpdated = await User.updateOne({ _id: msg.user_id }, { $pull: { [userTargetArray]: msg.list_id } });
                listUpdated = await List.updateOne({ _id: msg.list_id }, { $pull: { [listTargetArray]: msg.user_id } });
